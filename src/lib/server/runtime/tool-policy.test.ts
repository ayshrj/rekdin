...
describe("resolveAllowedToolNames", () => {
  it("blocks destructive tools in read-only mode", () => {
    ...
    expect(tools).toContain("session_inspect")
    expect(tools).toContain("replay_summary")
    expect(tools).toContain("replay_search")
    expect(tools).toContain("trace_summary")
    expect(tools).toContain("token_usage_report")
    expect(tools).toContain("background_jobs_summary")
    expect(tools).toContain("settings_summary")
  ...
  it("requires approval for risky balanced tools and host-level full-auto tools", () => {
    ...
    expect(requiresToolApproval("replay_summary", "balanced")).toBe(false)
    expect(requiresToolApproval("settings_summary", "full_auto")).toBe(false)
